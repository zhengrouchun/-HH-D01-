 ...
 * 2025-12-17     Luo Weilong  Add RT-Thread adaptation layer code for use with LiteOS.
 */
...
LOS_DL_LIST g_rtDeviceList = {NULL, NULL};
typedef struct {
    LOS_DL_LIST devItem;
    rt_device_t dev;
#if RT_NAME_MAX > 0
...
typedef rtDeviceNode *RtDeviceNode;
                            ...
                            rt_uint16_t flags)
{
    ...
    if (g_rtDeviceList.pstNext == NULL && g_rtDeviceList.pstPrev == NULL) {
        LOS_ListInit(&g_rtDeviceList);
    }
    RtDeviceNode node = (RtDeviceNode)LOS_MemAlloc(m_aucSysMem1, sizeof(rtDeviceNode));
    dev->flag = flags;
    dev->ref_count = 0;
    dev->open_flag = 0;

#if RT_NAME_MAX > 0
    rt_strncpy(node->name, name, RT_NAME_MAX);
#else
    node->name = name;
#endif
    ...
    node->dev = dev;
    LOS_ListTailInsert(&g_rtDeviceList, &node->devItem);
    LOS_MuxPost(g_MUX);
    return RT_EOK;
}
...
rt_err_t rt_device_unregister(rt_device_t dev)
{
    ...
    RtDeviceNode devNode = NULL;
    LOS_MuxPend(g_MUX, 10);
    LOS_DL_LIST_FOR_EACH_ENTRY(devNode, &g_rtDeviceList, rtDeviceNode, devItem) {
        if (devNode->dev == dev) {
            LOS_ListDelete(&(devNode->devItem));
            LOS_MemFree(m_aucSysMem1, dev);
            return RT_EOK;
        }
    }
    LOS_MuxPost(g_MUX);
...
rt_device_t rt_device_find(const char *name)
{
    ...
    RtDeviceNode devNode = NULL;
    if (g_rtDeviceList.pstNext == NULL) {
        return RT_NULL;
    }

    LOS_DL_LIST_FOR_EACH_ENTRY(devNode, &g_rtDeviceList, rtDeviceNode, devItem) {
        if (rt_strcmp(devNode->name, name) == 0) {
            return devNode->dev;
        }
    }
    return RT_NULL;
