...
#ifndef CONFIG_VFS_DISABLE_REMOVE_OLDINODE
...
next_subdir:
  ...
  ret = OK;
#ifndef CONFIG_VFS_ENABLE_SAME_DIR_RENAME
  if (strcmp(oldrelpath, newrelpath) != 0)
#endif
    {
...
#ifdef CONFIG_VFS_FULLPATH_ACQUIRE
  ret = vfs_acquire_fullpath(oldpath, &fulloldpath);
  if (ret < 0)
   {
     goto errout;
   }

  ret = vfs_acquire_fullpath(newpath, &fullnewpath);
  if (ret < 0)
   {
     goto errout_with_oldpath;
   }

  /* Root shall not be renamed */
  if (fulloldpath[0] == '/' && fulloldpath[1] == '\0')
    {
      ret = -EACCES;
      goto errout_with_prefix;      
    }

  /* According to POSIX, The new pathname shall not contain a path prefix that names old. */
  prefix = strlen(fulloldpath);
  if (strncmp(fulloldpath, fullnewpath, prefix) == 0 && fullnewpath[prefix] == '/')
    {
      ret = -EACCES;
      goto errout_with_prefix;
    }
...
#ifdef CONFIG_VFS_FULLPATH_ACQUIRE
errout_with_prefix:
  vfs_release_fullpath(fullnewpath);
errout_with_oldpath:
  vfs_release_fullpath(fulloldpath);
#endif
