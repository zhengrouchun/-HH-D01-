                      ...
                      int oflags, mode_t umask, va_list ap)
{
...
#endif
#ifdef CONFIG_VFS_FULLPATH_ACQUIRE
  ret = vfs_acquire_fullpath(path, &fullpath);
  if (ret < 0)
  {
    goto errout;
  }
...
#ifndef CONFIG_VFS_DISABLE_BLOCK_DEVICE
...
#ifndef CONFIG_VFS_DISABLE_INODE_CHECKFLAGS
  ...
  /* Allocate a new file descriptor for the inode,  we alloc fd
   * here to avoid incorrect open(O_CREAT) file below if The space
   * of file isn't enough.
   */

  fd = files_allocate(filep->f_inode, filep->f_oflags,
                      filep->f_pos, filep->f_priv, 0, filep);
  if (fd < 0)
    {
      ret = -EMFILE;
      goto errout_with_inode;
...
{
  return open (__path, ((unsigned int)__oflag) | O_LARGEFILE, 0666);
}
#endif
