...
#ifdef CONFIG_VFS_POSIX_OFLAGS
  if (vfs_posix_oflags_is_readonly(filep->f_oflags))
#else
  ...
  if (ret >= 0)
    {
       ...
       */
#ifdef CONFIG_VFS_WRITE_EXTEND
      if (vfs_posix_oflags_with_append(filep->f_oflags))
        {
          if (vfs_seek_check(filep))
            {
              return -EINVAL;
            }
        }
...
ssize_t write(int fd, FAR const void *buf, size_t nbytes)
{
  ...
  if (vfs_socket_fd_verify(fd))
    {
      ret = vfs_write_socket(fd, buf, nbytes);
  ...
  if (vfs_console_fd_verify(fd) && vfs_console_update_fd(&fd) != OK)
    {
      set_errno(EBADF);
      leave_cancellation_point();
      return ERROR;
