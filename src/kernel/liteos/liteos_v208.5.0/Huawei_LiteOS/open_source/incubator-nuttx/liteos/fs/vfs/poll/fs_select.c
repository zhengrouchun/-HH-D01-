           ...
           FAR fd_set *exceptfds, FAR struct timeval *timeout)
{
  ...
  if (npfds != 0)
    {
      ret = vfs_optimization_alloc_pollfd(&pollset, pfd, npfds, &pfd_alloc_flag);
      if (ret < 0)
        {
    ...
    {
    /* If the readfds, writefds, and exceptfds arguments are all null pointers and the
     * timeout argument is not a null pointer, the select() function shall block for
     * the time specified.
     */

      ret = vfs_select_wait(timeout);
      leave_cancellation_point();
      return ret;
    }
#else
  ...
  for (fd = 0, ndx = 0; fd < nfds; fd++)
    {
      ...
      if (readfds && FD_ISSET(fd, readfds))
        {
          pollset[ndx].fd      = fd;
#ifdef CONFIG_VFS_POLLSET_EVENT_EXTEND
          pollset[ndx].events |= vfs_pollfd_events_get(POLLIN);
#else
      ...
      if (writefds && FD_ISSET(fd, writefds))
        {
          pollset[ndx].fd      = fd;
#ifdef CONFIG_VFS_POLLSET_EVENT_EXTEND
          pollset[ndx].events |= vfs_pollfd_events_get(POLLOUT);
#else
      ...
      for (ndx = 0; ndx < npfds; ndx++)
        {
          ...
          if (readfds)
            {
#ifdef CONFIG_VFS_POLLSET_EVENT_EXTEND
              if (vfs_revents_pollset_check(pollset[ndx].revents, POLLIN))
#else
          ...
          if (writefds)
            {
#ifdef CONFIG_VFS_POLLSET_EVENT_EXTEND
              if (vfs_revents_pollset_check(pollset[ndx].revents, POLLOUT))
#else
          ...
          if (exceptfds)
            {
#ifdef CONFIG_VFS_POLLSET_EVENT_EXTEND
              if (vfs_revents_pollset_check(pollset[ndx].revents, POLLERR))
#else
...
#ifdef CONFIG_VFS_OPTIMIZATION_ALLOC_POLLFD
  vfs_optimization_free_pollfd(pollset, pfd_alloc_flag);
  leave_cancellation_point();
  return ret;
#else
  kmm_free(pollset);
