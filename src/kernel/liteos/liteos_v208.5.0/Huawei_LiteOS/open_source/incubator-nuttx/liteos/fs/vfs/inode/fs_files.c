...
#ifndef CONFIG_VFS_FILES_EXTEND_DISABLE
      ...
      for (j = CONFIG_NFILE_DESCRIPTORS_PER_BLOCK - 1; j >= 0; j--)
        {
          ...
          file_close(&list->fl_files[i][j], true);
#else
...
#ifdef CONFIG_FILES_ALLOCATE_WITH_FILEP
              vfs_filep_path_set(&list->fl_files[i][j], filep);
#endif
              _files_semgive(list);
...
#ifdef CONFIG_FILES_ALLOCATE_WITH_FILEP
      vfs_filep_path_set(&list->fl_files[i][0], filep);
#endif
