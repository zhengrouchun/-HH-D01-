 ...
 *
 ****************************************************************************/
#ifdef CONFIG_VFS_UMOUNT_WITHOUT_FLAGS
int nx_umount2(const char *target)
#else
...
#endif
{
...
#ifdef CONFIG_VFS_FULLPATH_ACQUIRE
  ret = vfs_acquire_fullpath(target, &fullpath);
  if (ret < 0)
    {
      goto errout;
    }
  ...
  /* Verify the path is a mountpoint path or file path*/
#ifdef CONFIG_VFS_DESC_PATH_VERIFY
  ...
  ret = vfs_mountpt_inode_unbind(&mountpt_inode, &blkdrvr_inode);
  if (ret < 0)
    {
...
errout_with_mountpt:
#ifdef CONFIG_VFS_CHANGE_REFERENCE_COUNT
  mountpt_inode->i_crefs--;
#else
  inode_release(mountpt_inode);
#endif
