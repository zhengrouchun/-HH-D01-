...
/**
 *    @file
 *          Platform-specific key value storage implementation for Hisi
 */
...
public:
    ...
    CHIP_ERROR _Get(const char * key, void * value, size_t value_size,
                    size_t * read_bytes_size = nullptr, size_t offset = 0);

    CHIP_ERROR _Delete(const char * key);

    CHIP_ERROR _Put(const char * key, const void * value, size_t value_size);

    CHIP_ERROR _PutEncrypted(const char * key, const void * value, size_t value_size);

    CHIP_ERROR _GetEncrypted(const char * key, void * value, size_t value_size, size_t * read_bytes_size = nullptr);
...
private:
    ...
    // 添加缺失的方法声明
    uint16_t FindKeyIDByKeyName(const char * key);
    nv_key_attr_t GetKeyAttributes(uint16_t keyID);
