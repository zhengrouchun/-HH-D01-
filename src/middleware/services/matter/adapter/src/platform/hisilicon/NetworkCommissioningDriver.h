...
#define SECURITYCONVERT(security)  (((security) < 2) ? ((security) + 1) : ((security) == 2) ? 8 : 4)
...
class HisiScanResponseIterator : public Iterator<WiFiScanResponse> {
public:
    HisiScanResponseIterator(const size_t size, const wifi_scan_info_stru * scanResults)
    ...
    bool Next(WiFiScanResponse & item) override
    {
        ...
        item.security.SetRaw(SECURITYCONVERT(mpScanResults[mIternum].security_type));
        item.ssidLen = static_cast<uint8_t>(strnlen(reinterpret_cast<const char *>(mpScanResults[mIternum].ssid),
            chip::DeviceLayer::Internal::kMaxWiFiSSIDLength));
        item.channel = mpScanResults[mIternum].channel_num;
        item.wiFiBand = chip::DeviceLayer::NetworkCommissioning::WiFiBand::k2g4;
        item.rssi     = mpScanResults[mIternum].rssi;
        memcpy_s(item.ssid, WIFI_MAX_SSID_LEN, mpScanResults[mIternum].ssid, item.ssidLen);
        memcpy_s(item.bssid, WIFI_MAC_LEN, mpScanResults[mIternum].bssid, 6); // 6 is the length of wifi mac
...
private:
    const size_t mSize;
    const wifi_scan_info_stru * mpScanResults;
    size_t mIternum = 0;
};

class HisiWiFiDriver final : public WiFiDriver {
public:
    class WiFiNetworkIterator final : public NetworkIterator {
    public:
        explicit WiFiNetworkIterator(HisiWiFiDriver * aDriver) : mDriver(aDriver) {}
    ...
    static HisiWiFiDriver & GetInstance()
    {
        static HisiWiFiDriver instance;
        return instance;
    }
