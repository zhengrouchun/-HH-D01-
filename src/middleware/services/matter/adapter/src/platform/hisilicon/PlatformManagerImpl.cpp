...
#include <platform/hisilicon/SystemTimeSupport.h>
...
{
    ChipLogProgress(DeviceLayer, "HisiPlatformManagerImpl::_InitChipStack start");
    CHIP_ERROR err;
    // 初始化NV存储系统
    uapi_nv_init();
    ChipLogProgress(DeviceLayer, "NV storage system initialized");
    MatterAtCmdRegister();
    ...
    SuccessOrExit(err);
    ChipLogProgress(DeviceLayer, "HisiPlatformManagerImpl::_InitChipStack OK");
exit:
    return err;
}
