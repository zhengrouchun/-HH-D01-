...
{
    uint64_t tickCount = osKernelGetTickCount();
    uint64_t milliseconds = (tickCount / LOSCFG_BASE_CORE_TICK_PER_SECOND) * kMillisecondsPerSecond;

    return (Clock::Microseconds64(milliseconds * kMicrosecondsPerMillisecond));
}
...
Milliseconds64 ClockImpl::GetMonotonicMilliseconds64(void)
{
    ...
    return (Clock::Milliseconds64(milliseconds));
}
...
{
    baseTime = Clock::Milliseconds64(
        (static_cast<uint64_t>(CHIP_SYSTEM_CONFIG_VALID_REAL_TIME_THRESHOLD) * kMicrosecondsPerMillisecond));
