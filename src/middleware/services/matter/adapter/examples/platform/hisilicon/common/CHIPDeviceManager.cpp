...
#include <platform/hisilicon/HisiFactoryDataProvider.h>
...
chip::DeviceLayer::HisiFactoryDataProvider mFactoryDataProvider;
...
{
    ChipLogProgress(DeviceLayer, "CHIPDeviceManager::Init start");
    CHIP_ERROR err;
    mCB = cb;
    ...
    ChipLogProgress(DeviceLayer, "CHIPDeviceManager::Init OK");
exit:
    return err;
}
} // namespace DeviceManager
...
void MatterPostAttributeChangeCallback(
    const chip::app::ConcreteAttributePath & path, uint8_t type, uint16_t size, uint8_t * value)
{
