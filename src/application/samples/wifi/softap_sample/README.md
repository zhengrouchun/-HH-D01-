...
本示例演示 WiFi SoftAP（Software Access Point）模式，设备作为热点供其他设备连接。
  ...
  └── Application
        └── Enable Sample  →  [*] (设为 y)
              └── Enable the Sample of WIFI.  →  [*] (设为 y)
                    └── Sample  →  选择 Support WIFI SoftAP Sample
                          └── SoftAP Sample Configuration
...
| `WIFI_PWD` | SoftAP 热点的密码 | `my_password` |

## 使用步骤

1. 通过 `python build.py menuconfig` 或 HiSpark IDE 配置 SoftAP 的 SSID 和密码
2. 编译并烧录固件
3. 设备启动后会自动创建 WiFi 热点
4. 其他设备可搜索并连接到该热点（默认 IP: 192.168.43.1）
