# RADAR WIRELESS Sample

## 功能说明

本示例演示雷达通过 WiFi STA 模式连接至指定 AP，并通过 Socket 传输雷达数据。
  ...
  └── Application
        └── Enable Sample  →  [*] (设为 y)
              └── Enable the Sample of RADAR.  →  [*] (设为 y)
                    └── Support RADAR WIRELESS Sample  →  [*] (选中)
                          └── RADAR WIRELESS Sample Configuration
...
2. 编译并烧录固件
3. 设备启动后会自动连接 WiFi 并通过 Socket 传输雷达数据
