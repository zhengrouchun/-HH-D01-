...
// 打印 station 信息
static void PrintStationInfo(const wifi_sta_info_stru *info) 
{
  if (!info) return;
  static char macAddress[32] = {0};
  const unsigned char* mac = info->mac_addr;
...
)
{
    ...
    osal_printk("OnWifiConnectionChanged state=%d\r\n", state);
...
static void OnSoftAPStaJoin(const wifi_sta_info_stru *info)
{
...
static void OnSoftAPStaLeave(const wifi_sta_info_stru *info)
{
  g_joinedStations--;
  PrintStationInfo(info);
