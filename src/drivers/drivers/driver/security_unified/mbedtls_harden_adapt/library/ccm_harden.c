...
void mbedtls_ccm_init_harden(mbedtls_ccm_context *ctx)
{
    ...
    for (i = 0; i < MAX_CCM_HARD_CTX_NUM; ++i) {
        if (g_ccm_harden_context[i].is_used == 0) {
            (void)memset_s(&g_ccm_harden_context[i], sizeof(ccm_harden_context), 0, sizeof(ccm_harden_context));
            (void)memset_s(g_ccm_harden_context[i].key, sizeof(g_ccm_harden_context[i].key), 0,
                sizeof(g_ccm_harden_context[i].key));
    ...
    unsigned int keybits)
{
    ...
    if (ctx == NULL || key == NULL)
        return -1;
    if (keybits != 128 && keybits != 192 && keybits != 256) // 128, 192, 256: key bits
        return -1;
    ...
    (void)memcpy_s(harden_ctx->key, sizeof(harden_ctx->key), key, harden_ctx->key_length);
...
void mbedtls_ccm_free_harden(mbedtls_ccm_context *ctx)
{
    ...
    if (harden_ctx != NULL) {
        ...
        KM_DEINIT();
        CIPHER_SYMC_DEINIT();
        (void)memset_s(harden_ctx, sizeof(ccm_harden_context), 0, sizeof(ccm_harden_context));
    }
}
...
int mbedtls_ccm_update_ad_harden(mbedtls_ccm_context *ctx, const unsigned char *add, size_t add_len)
{
    ...
    if (harden_ctx != NULL && harden_ctx->only_software_support == 0) {
        ...
        (void)memcpy_s(harden_ctx->aad + harden_ctx->cur_aad_len, sizeof(harden_ctx->aad) - harden_ctx->cur_aad_len,
            add, add_len);
...
{
    if (iv_len < 7 || iv_len > 13)  // 7, 13: min and max iv_len
        return (MBEDTLS_ERR_CCM_BAD_INPUT);
    if (ctx == NULL || iv == NULL)
        return (MBEDTLS_ERR_CCM_BAD_INPUT);
...
{
    if (tag_len == 2 || tag_len > 16 || tag_len % 2 != 0)   // 2, 16: algo require
        return (MBEDTLS_ERR_CCM_BAD_INPUT);
