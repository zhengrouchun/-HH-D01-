...
static int inner_alloc_pub_key(const mbedtls_rsa_context *ctx, drv_pke_rsa_pub_key *pub_key)
{
    ...
    buffer = mbedtls_calloc(2, size_align); // 2: 2 buffers
    if (buffer == NULL) {
...
cleanup:
    if (buffer != NULL) {
        mbedtls_platform_zeroize(buffer, 2 * size_align);   // 2: 2 buffers
        mbedtls_free(buffer);
    }
...
{
    if (pub_key->n != NULL) {
        mbedtls_platform_zeroize(pub_key->n, 2 * pub_key->len); // 2: 2 buffers
    ...
    drv_pke_data *out_buf)
{
    ...
    size_align = ctx->len;
    inner_get_klen(&size_align);
    buffer = mbedtls_calloc(3, size_align); // 3: 3 buffers
    if (buffer == NULL) {
        mbedtls_harden_log_err("mbedtls_calloc failed!\n");
        return MBEDTLS_ERR_MPI_ALLOC_FAILED;
    }
    ...
    if (out_buf != NULL) {
        out_buf->data = buffer + 2 * size_align;    // 2: 2 buffers
...
cleanup:
    if (buffer != NULL) {
        mbedtls_platform_zeroize(buffer, 3 * size_align);   // 3: 3 buffers
        mbedtls_free(buffer);
    }
...
{
    if (priv_key->n != NULL) {
        mbedtls_platform_zeroize(priv_key->n, 3 * priv_key->n_len); // 3: 3 buffers
    ...
    unsigned int label_len, const unsigned char *input, unsigned int ilen, unsigned char *output, unsigned int olen)
{
    ...
    if (ctx->padding == MBEDTLS_RSA_PKCS_V15) {
        ...
        ret = CIPHER_PKE_RSA_PUBLIC_ENCRYPT(DRV_PKE_RSA_SCHEME_PKCS1_V21, drv_hash_type, &pub_key, &input_data,
            &label_data,
            &output_data);
    }
    mbedtls_harden_log_func_exit();
