...
int check_exp_mod_harden_can_do(const mbedtls_mpi *A, const mbedtls_mpi *E, const mbedtls_mpi *N)
{
    ...
    return ((size_max - size_n) <= 2);  // 2: max offset bit
#else
    return (1);
#endif
}
...
int exp_mod_harden(mbedtls_mpi *X, const mbedtls_mpi *A, const mbedtls_mpi *E, const mbedtls_mpi *N)
{
    ...
    for (size_t i = 0; i < 3; i++) {   // 3: 3 buffers
    ...
    ret = CIPHER_PKE_EXP_MOD(&hw_data[2], &hw_data[1], &hw_data[0], &hw_data[0]);   // 2: idx
    if (ret != MBEDTLS_HARDEN_SUCCESS) {
        ret = MBEDTLS_ERR_PLATFORM_HW_ACCEL_FAILED;
        goto cleanup;
    }
...
int mod_harden(mbedtls_mpi *R, const mbedtls_mpi *A, const mbedtls_mpi *B)
{
    ...
    ret = CIPHER_PKE_MOD(&hw_data[0], &hw_data[1], &hw_data[2]);    // 2: idx
    if (ret != MBEDTLS_HARDEN_SUCCESS) {
    ...
    MBEDTLS_MPI_CHK(mbedtls_mpi_read_binary(R, hw_data[2].data, hw_data[2].length));    // 2: idx
