...
int ecdh_harden(mbedtls_ecp_group *grp, mbedtls_mpi *z, const mbedtls_ecp_point *Q, const mbedtls_mpi *d,
    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    int ret;
    drv_pke_data priv_key;
    ...
    byte_length = grp->pbits / 8 + ((grp->pbits % 8) != 0); // 8: 8 bits in byte
    ...
    */
        ...
        MBEDTLS_MPI_CHK(mbedtls_mpi_read_binary(z, shared_key.data + 2, byte_length));  // 2: algo require
    else
...
{
    if (check_ecc_harden_can_do(grp, f_rng) != 1 || blen < 20 || blen > 64) // 20, 64: algo require
        return (0);
...
{
    if (check_ecc_harden_can_do(grp, NULL) != 1 || blen < 20 || blen > 64 ||    // 20, 64: algo require
