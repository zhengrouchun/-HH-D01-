...
static void *bh1750_test(const char *arg){
    unused(arg);
    uapi_pin_init();
    uapi_gpio_init();
    ...
    while (1) {
        ...
        ssd1306_printf("%d Lx", lightness); // 显示光照亮度(Lx)
...
static void test_entry(void)
{
    ...
    osal_kthread_lock();
    // 创建传感器测试任务
    taskid_sensor = osal_kthread_create((osal_kthread_handler)bh1750_test, NULL, "bh1750_test", SENSOR_TASK_STACK_SIZE);
