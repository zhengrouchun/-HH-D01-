...
static void *ins5699s_test(const char *arg){
    unused(arg);
    uapi_pin_init();
    uapi_gpio_init();
...
static void test_entry(void)
{
    ...
    osal_kthread_lock();
    // 创建传感器测试任务
    taskid_sensor = osal_kthread_create((osal_kthread_handler)ins5699s_test, NULL, "ins5699s_test", SENSOR_TASK_STACK_SIZE);
