 ...
 * Copyright (c) kidwjb
 * Description: sle uuid server sample,cloud server control the LED.
 */
#include "securec.h"
#include "errcode.h"
#include "osal_addr.h"
...
{
    while (1) {
        LED_data[INDEX2] = (uint8_t)uapi_gpio_get_output_val((GPIO_02));
...
static void blinky_entry(void)
{
    ...
    osal_kthread_lock();
    task_handle =
        osal_kthread_create((osal_kthread_handler)LED_Control_report_task, 0, "BlinkyTask", BLINKY_TASK_STACK_SIZE);
    if (task_handle != NULL) {
